...
public class PageInfo extends Sprite{
    ...
    {
        if(this.contains(pageNumberField)) this.removeChild(pageNumberField);
        if(this.contains(pageThemeField)) this.removeChild(pageThemeField);


        var font:Font = new Georgia();
        var textLeft:String = "iTravel | " + appModel.currentPage.theme;
